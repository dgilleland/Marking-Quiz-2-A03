...
USE [Q2_Fall_2022]
GO
---remember, use "having" for aggregate stuff, and subquery with aggregates and "where" clause
--Drop Tables
...
LEFT OUTER JOIN Deliveries AS D 
    ON O.DeliveryNumber = D.DeliveryNumber
Where 
...
GO
SELECT V.DriverFirstName + ' ' + V.DriverLastName AS 'FullName',
V.VehicleNumber,
D.DeliveryNumber
FROM Deliveries AS D
LEFT OUTER JOIn Vehicles AS V
ON D.VehicleNumber = D.VehicleNumber
WHERE DeliveryDate LIKE '%-09-%'
...
R.GSTAmount
FROM OrderDetails AS O
LEFT OUTER JOIN Orders AS R 
ON O.OrderNumber = R.OrderNumber
GROUP BY O.OrderNumber, R.GSTAmount
HAVING ProductNumber = 1701
...
WHERE PostalCode BETWEEN 'T5_____' AND 'T9_____'
