...
ALTER TABLE Deliveries
    ...
    CONSTRAINT FK_Deliveries_PrepaidCollect FOREIGN KEY REFERENCES
    CHECK 
...
-- ANSWER_HERE
SELECT Description, SUM(Quantity) 'Sum of Quantities Sold'
...
-- ANSWER_HERE
SELECT DriverFirstName + ' ' + DriverLastName AS 'FullName',  V.VehicleNumber, DeliveryNumber
FROM Vehicles as V 
...
-- 4. (4 Marks) We are anticipating a drop in the GST by the federal government. To understand the impact of such a drop, write a query showing [totals for each S: order]; only include orders containing the [W: product number 1701]. Return the [order number], the [current GST] (the sum of `selling price X quantity X 5%`), and the proposed GST (the sum of `selling price X quantity X 4%`).
-- ANSWER_HERE
SELECT SUM(O.OrderNumber) as 'OrderTotal', SUM(SellingPrice * Quantity * 0.05) AS 'CurrentGST'
FROM Orders as O
...
-- ANSWER_HERE
SELECT City,  SUM(CustomerNumber) AS 'Customers'
FROM Customers
...
-- ANSWER_HERE
CREATE OR ALTER VIEW ViewProductOrders
AS 
...
FROM ViewProductOrders
GROUP BY [Description]
HAVING AVG(SellingPrice) IS NOT NULL
...
-- ANSWER_HERE
UPDATE Products
SET CurrentPrice = CurrentPrice - CurrentPrice * 0.15
...
-- ANSWER_HERE
DELETE Customer
WHERE CustomerNumber NOT IN (SELECT OrderNumber FROM Orders)
