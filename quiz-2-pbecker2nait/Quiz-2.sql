...
(
    ...
    DeliveryNumber int
        ...
        NOT NULL,
    DeliveredYN char(1) DEFAULT 'N' NOT NULL,
...
CREATE NONCLUSTERED INDEX [IX_Orders_DeliveryNumber] ON [Orders]([DeliveryNumber])
GO
...
ALTER TABLE [Deliveries]
    ADD [PrepaidCollect] char NULL
    CONSTRAINT [CK_Deliveries_PrepaidCollect]
...
SELECT [Description], SUM([Quantity]) AS [QuantitySold]
FROM [OrderDetails]
INNER JOIN [Products] ON [OrderDetails].[ProductNumber] = [Products].[ProductNumber]
GROUP BY [OrderDetails].[ProductNumber], [Description]
...
SELECT [DriverFirstName] + ' ' + [DriverLastName] AS [Driver],
    [Deliveries].[VehicleNumber], [DeliveryNumber]
FROM [Deliveries]
INNER JOIN [Vehicles] ON [Deliveries].[VehicleNumber] = [Vehicles].[VehicleNumber]
WHERE YEAR([DeliveryDate]) = YEAR(GETDATE()) AND MONTH([DeliveryDate]) = 9
GO
...
SELECT [OrderNumber],
    SUM([Quantity] * [SellingPrice] * 0.05) AS [Current GST],
    SUM([Quantity] * [SellingPrice] * 0.04) AS [Proposed GST]
FROM [OrderDetails]
...
AS
    SELECT [Products].[ProductNumber], [Description], [CurrentPrice],
        [OrderNumber], [Quantity], [SellingPrice]
    FROM [Products]
    LEFT JOIN [OrderDetails] ON [Products].[ProductNumber] = [OrderDetails].[ProductNumber]
GO
...
SELECT [Description], AVG([SellingPrice]) AS [AverageSellingPrice]
FROM [ProductOrders]
WHERE [ProductNumber] IN
(
    SELECT DISTINCT [ProductNumber] FROM [OrderDetails]
)
