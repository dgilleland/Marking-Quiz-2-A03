...
-- ANSWER_HERE
SELECT  DriverFirstName + ' ' + DriverLastName AS "Driver's Full Name",
        V.VehicleNumber,
        DeliveryNumber
FROM    Vehicles AS V
    LEFT OUTER JOIN Deliveries AS D
        ON V.VehicleNumber = D.VehicleNumber
WHERE   DATEPART(MONTH, DeliveryDate) = 9
GO
...
-- ANSWER_HERE
SELECT  OrderNumber,
        SellingPrice * Quantity * 1.05 AS 'Current GST',
        SellingPrice * Quantity * 1.04 AS 'Proposed GST'
FROM    OrderDetails AS OD
...
-- ANSWER_HERE
SELECT  City,
        COUNT(C.CustomerNumber) AS 'Total Number of Orders'
FROM    Orders AS O
...
FROM    Products
WHERE   [Description] LIKE 'C%'
-- Carnation 1.50 - 1.28, Cut Flowers 5.50 -> 4.68, Cactus 47.50 -> 40.38
*/
