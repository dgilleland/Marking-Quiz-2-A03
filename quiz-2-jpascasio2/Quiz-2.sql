...
(
    ...
    PaidYN              char(1)
        CONSTRAINT CK_Orders_PaidYN DEFAULT ('N')
                                         NOT NULL
)
...
ALTER TABLE Deliveries
    ADD PrepaidCollect char(1)
        CONSTRAINT CK_Deliveries_PrepaidCollect
            CHECK (PrepaidCollect LIKE '[PC]') NULL
GO
...
-- ANSWER_HERE SELECT * FROM OrderDetails
SELECT P.Description,
    SUM(OD.Quantity)
FROM Products AS P
    INNER JOIN OrderDetails AS OD ON P.ProductNumber = OD.ProductNumber
...
-- ANSWER_HERE
SELECT DriverFirstName + ' ' + DriverLastName AS 'Driver Name', 
    D.VehicleNumber,
    D.DeliveryNumber
FROM Vehicles AS V
...
WHERE DATENAME(mm, O.OrderDate) = 'September' AND DATEPART(yyyy, O.OrderDate) = GETDATE()
...
-- ANSWER_HERE SELECT * FROM Customers
SELECT City,
    COUNT(O.CustomerNumber) AS 'Customers'
FROM Customers AS C
...
-- ANSWER_HERE SELECT * FROM ProductOrders
SELECT Description,
...
-- ANSWER_HERE SELECT * FROM Products
UPDATE Products
