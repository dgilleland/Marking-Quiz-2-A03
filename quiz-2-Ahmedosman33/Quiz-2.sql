...
(
    ...
    PostalCode          char(6)     
    CONSTRAINT CK_Customers_PostalCode CHECK(PostalCode like '[A-Z][0-9][A-Z][0-9][A-Z][0-9][A-Z][0-9]')
                                    NOT NULL,
...
CONSTRAINT CK_Deliveries_PrepaidCollect CHECK(PrepaidCollect LIKE '[PC]')
...
INNER JOIN Products AS P ON O.ProductNumber = P.ProductNumber
...
-- ANSWER_HERE
SELECT DriverFirstName + ' ' + DriverLastName AS 'Driver Full Name', D.VehicleNumber, DeliveryNumber, DeliveryDate
FROM Vehicles AS V
LEFT OUTER JOIN Deliveries AS D ON V.VehicleNumber = D.VehicleNumber
...
FROM Customers 
WHERE Province = 'AB' AND PostalCode BETWEEN 'T5%' AND 'T9%' 
GROUP BY City
...
FROM Products AS P
INNER JOIN OrderDetails AS O ON O.ProductNumber = P.ProductNumber
