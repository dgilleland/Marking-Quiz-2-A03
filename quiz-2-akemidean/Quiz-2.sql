...
-- ANSWER_HERE
SELECT O.ProductNumber, P.Description
FROM Products AS P
    INNER JOIN OrderDetails AS O
       ON O.ProductNumber = P.ProductNumber
       GROUP by O.ProductNumber, P.[Description]
     HAVING O.Quantity > 275  
GO
...
-- ANSWER_HERE
SELECT DriverFirstName, + DriverLastName AS 'Driver Full Name'
FROM Vehicles as V  
 INNER JOIN Deliveries D
    ON V.VehicleNumber = D.VehicleNumber
WHERE D.DeliveryDate = 'September 2022'
GO
...
SELECT City
From Customers
WHERE PostalCode = 'T5' OR PostalCode = 'T9'
GO
...
SELECT  AVG(SellingPrice)
 WHERE ProductOrders is not NULL
GO
...
-- ANSWER_HERE
DELETE FROM Customers
WHERE CustomerNumber NOT IN (SELECT OrderNumber FROM OrderDetails GROUP BY OrderNumber) 
GO
