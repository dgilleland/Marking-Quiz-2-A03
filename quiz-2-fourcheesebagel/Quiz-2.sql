...
-- ANSWER_HERE
SELECT SUM(Quantity) AS 'Total Quantities Ordered', [Description]
...
-- ANSWER_HERE
SELECT DriverFirstName + DriverLastName AS 'Driver Full Name', V.VehicleNumber, O.DeliveryNumber
FROM Vehicles AS V
    INNER JOIN Deliveries AS D
        ON V.VehicleNumber = D.VehicleNumber
    INNER JOIN Orders AS O
        ON D.DeliveryNumber = O.DeliveryNumber
    WHERE (DATEPART(mm,OrderDate) = 9) AND (DATEPART(yy,OrderDate) = DATEPART(yy,GETDATE())) 
GO
...
-- ANSWER_HERE
SELECT SUM(Quantity * SellingPrice) AS 'Total for each order', OrderNumber
FROM OrderDetails
WHERE ProductNumber = 1701
GROUP BY OrderNumber
GO

SELECT OrderNumber,SUM(Quantity * SellingPrice * 0.05) AS 'Current GST', SUM(Quantity * SellingPrice * 0.04) AS 'Proposed GST'
FROM OrderDetails
WHERE ProductNumber = 1701
GROUP BY OrderNumber
GO
...
-- ANSWER_HERE
SELECT City, SUM(CustomerNumber) AS 'Total Number of Customers'
FROM Customers
...
AS
SELECT P.ProductNumber, [Description], CurrentPrice, OrderNumber, Quantity, SellingPrice
FROM   Products AS P
