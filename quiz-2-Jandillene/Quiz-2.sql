...
(
    ...
    PostalCode          char(6)     
        ...
        CHECK(PostalCode LIKE '[A-Z][1-9][A-Z][1-9][A-Z][1-9]')
                                    NOT NULL,
...
-- ANSWER_HERE
CREATE NONCLUSTERED INDEX IX_OrderNumber_OrderDetails
ON OrderDetails
GO

CREATE NONCLUSTERED INDEX IX_ProductNumber_OrderDetails
ON OrderDetails
GO
...
-- ANSWER_HERE
GO
SELECT P.Description, SUM(OD.Quantity) AS 'Total Quantity Ordered'
FROM Products AS P
LEFT OUTER JOIN OrderDetails AS OD ON P.ProductNumber = OD.ProductNumber
GROUP BY P.Description
HAVING SUM(OD.Quantity) > 275
GO
...
-- ANSWER_HERE
SELECT DriverFirstName + ' ' + DriverLastName AS 'Driver Full Name', D.VehicleNumber, DeliveryNumber
FROM Vehicles AS V
LEFT OUTER JOIN Deliveries AS D ON D.VehicleNumber = V.VehicleNumber
WHERE DATEPART(mm, DeliveryDate) = 09 AND YEAR(DeliveryDate) = YEAR(GETDATE())
GO 
...
-- ANSWER_HERE
SELECT OD.OrderNumber, (SellingPrice * Quantity) * 0.05 AS 'Current GST', (SellingPrice * Quantity) * 0.04 AS 'Proposed GST'
FROM OrderDetails AS OD
LEFT OUTER JOIN Orders AS O ON OD.OrderNumber = O.OrderNumber
WHERE OD.ProductNumber = 1701
GO
...
-- ANSWER_HERE
SELECT City, SUM(O.CustomerNumber) AS 'Total Customers'
FROM Customers AS C
LEFT OUTER JOIN Orders AS O ON C.CustomerNumber = O.CustomerNumber
WHERE PostalCode LIKE 'T5____' OR PostalCode LIKE 'T9____' AND OrderNumber IS NOT NULL
GROUP BY City
ORDER BY City
GO
...
AS
    SELECT P.ProductNumber AS 'Product Number',
        Description,
        CurrentPrice,
...
-- ANSWER_HERE
SELECT [Description], AVG(SellingPrice) AS 'Average Selling Price'
FROM ProductOrders
