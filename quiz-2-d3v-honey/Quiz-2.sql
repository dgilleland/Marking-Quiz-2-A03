...
ALTER TABLE Deliveries
    ...
    CONSTRAINT CK_Deliveries_PrepaidCollect
            CHECK (PrepaidCollect = 'Y' OR PrepaidCollect = 'N') NULL
GO
...
FROM Orders
WHERE DeliveredYN = 'N' AND PaidYN = 'Y'
...
SELECT SUM(Quantity) as 'Total Quantity', P.[Description]
FROM OrderDetails AS OD
    INNER JOIN Products AS P ON OD.ProductNumber = P.ProductNumber 
GROUP BY OD.ProductNumber, P.[Description]
HAVING SUM(SellingPrice) > 275
...
SELECT V.DriverFirstName +' '+ V.DriverLastName, V.VehicleNumber, D.DeliveryNumber  
FROM Vehicles as V
    INNER JOIN Deliveries as D on V.VehicleNumber = D.VehicleNumber
WHERE YEAR(D.DeliveryDate) = YEAR(GETDATE()) AND MONTH(D.DeliveryDate) = 9
...
SELECT C.City, SUM(O.CustomerNumber) AS 'Total Customer'
FROM Orders AS O
INNER JOIN Customers AS C ON C.CustomerNumber = O.CustomerNumber
WHERE SUBSTRING(C.PostalCode, 1, 2) IN ('T5', 'T6', 'T7', 'T8', 'T9')
GROUP BY C.City
ORDER BY C.City ASC
GO
-- PART E – Views
...
-- ANSWER_HERE
IF OBJECT_ID('ProductOrders', 'V') IS NOT NULL
    DROP VIEW ProductOrders
GO
CREATE VIEW ProductOrders
AS 
    SELECT P.ProductNumber, P.[Description], P.CurrentPrice, OD.OrderNumber, OD.Quantity, OD.SellingPrice
    FROM OrderDetails AS OD
    INNER JOIN Products AS P ON OD.ProductNumber = P.ProductNumber
GO
...
SELECT  [Description], AVG(SellingPrice)
FROM ProductOrders
...
-- ANSWER_HERE
UPDATE Products
SET CurrentPrice = CurrentPrice - (CurrentPrice *.15)
WHERE SUBSTRING([Description], 1, 1) = 'C'
GO
