...
ALTER TABLE Deliveries
    ADD PrepaidCollect CHAR(1) NULL
        CONSTRAINT CK_YES_NO
            CHECK (PrepaidCollect = 'Y' OR 
            PrepaidCollect = 'N')
GO
...
-- ANSWER_HERE
SELECT SUM(Quantity) as 'Amount Sold',
        [Description]
FROM OrderDetails AS OD
    LEFT OUTER JOIN Products AS P ON P.ProductNumber = OD.ProductNumber
GROUP BY OD.ProductNumber, [Description]
...
SELECT DeliveryNumber,
    DriverFirstName + ' ' + DriverLastName AS 'Driver Name'
FROM Deliveries AS D
    INNER JOIN Vehicles AS V on V.VehicleNumber = D.VehicleNumber
GROUP BY DeliveryDate, DeliveryNumber, DriverFirstName, DriverLastName
HAVING YEAR(DeliveryDate) = YEAR(CURRENT_TIMESTAMP) AND MONTH(DeliveryDate) = MONTH('Sep 01, 2000')
GO
...
SELECT O.OrderNumber,
        SUM(SellingPrice * Quantity) * 0.05 AS 'Current GST',
        SUM(SellingPrice * Quantity) * 0.04 AS 'Proposed GST'
...
GROUP BY O.OrderNumber, SellingPrice, Quantity, GSTAmount
...
FROM Customers
WHERE PostalCode LIKE 'T5%' OR PostalCode LIKE 'T9%'
GROUP BY City
ORDER BY City ASC
...
SELECT [Description],
        AVG(SellingPrice) AS 'Average Price'
FROM ProductOrders
