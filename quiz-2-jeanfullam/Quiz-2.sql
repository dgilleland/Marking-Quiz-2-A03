...
CREATE NONCLUSTERED INDEX IX_OrderdDetails_ProductNumber
...
ALTER TABLE Deliveries
    ...
    CHECK (PrepaidCollect = 'P' OR PrepaidCollect = 'C')
...
SELECT  OrderNumber, CustomerNumber, OrderDate, GSTAmount, DeliveredYN
FROM    Orders
WHERE   DeliveredYN = 'N'
GO
...
SELECT DriverFirstName +  ' ' +  DriverLastName AS 'Drivers Full Name',
       D.VehicleNumber,
       O.DeliveryNumber,
       OrderDate
FROM   Vehicles AS V
    LEFT OUTER JOIN Deliveries AS D
        ON V.VehicleNumber = D.VehicleNumber
    LEFT OUTER JOIN Orders AS O
        ON D.DeliveryNumber = O.DeliveryNumber
...
--write a query showing totals for each order; only include orders containing the product number 1701. Return the order number, 
--the current GST (the sum of `selling price X quantity X 5%`), and the proposed GST (the sum of `selling price X quantity X 4%`).
SELECT OrderNumber,
       SellingPrice * Quantity * 0.05 AS 'Current GST',
       Quantity * SellingPrice * 0.04 AS 'Proposed GST'
FROM   OrderDetails
WHERE  ProductNumber = '1701'

GO

-- 5. (3 Marks) We would like to know which city the majority of our northern Alberta customers are from.
-- List the city and the total number of customers who placed an order from that city with postal codes starting with T5 to T9. 
...
AS
    SELECT O.ProductNumber,
           ...
           SellingPrice
    FROM   Products AS P
        LEFT OUTER JOIN OrderDetails AS O
