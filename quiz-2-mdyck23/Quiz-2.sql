...
CREATE NONCLUSTERED INDEX IX_OrderDetails_ProductNumber
...
-- PART C - Alter
-- (2 marks) Assuming the tables all have data in them already, use the Alter Table statement to add a nullable column 
-- PrepaidCollect to the Deliveries table. The column will be one character, will be optional, and the only valid values 
-- will be 'P' (Prepaid) or 'C' Collect.
-- ANSWER_HERE

ALTER TABLE Deliveries
    ADD PrepaidCollect      char(1)     
        CONSTRAINT C_Deliveries_PrepaidCollect
            CHECK (PrepaidCollect LIKE 'P' OR PrepaidCollect LIKE 'C')
                                            NULL
...
-- 1. (2 Marks) List the order number, customer number, order date and GST amount for all the orders that were paid for 
-- but not delivered.
SELECT OrderNumber,
       CustomerNumber,
       OrderDate,
...
-- 2. (3 Marks) We want to know which products sell the most. Select the sum of the quantities sold for each product where 
-- the sum is greater than 275. Show the product description and the total quantity ordered.
-- ANSWER_HERE
SELECT  [Description],
        COUNT(OD.ProductNumber)
FROM Products AS P
    INNER JOIN OrderDetails AS OD
        ON P.ProductNumber = OD.ProductNumber
GROUP BY [Description]
HAVING COUNT(OD.ProductNumber) > 275

GO

-- 3. (2 Marks) A September delivery schedule is needed. Select the driver's full name (as one column), vehicle number 
-- and delivery number for all orders in September of the current year (do not hard code the current year).
-- ANSWER_HERE
SELECT  DriverFirstName + ' ' + DriverLastName AS 'Full Name',
        V.VehicleNumber
FROM Vehicles AS V
...
-- 4. (4 Marks) We are anticipating a drop in the GST by the federal government. To understand the impact of such a drop, 
-- write a query showing totals for each order; only include orders containing the product number 1701. Return the order 
-- number, the current GST (the sum of `selling price X quantity X 5%`), and the proposed GST 
-- (the sum of `selling price X quantity X 4%`).
-- ANSWER_HERE
SELECT  O.OrderNumber,
        (SellingPrice * Quantity * 1.05) AS 'Price with Current GST',
        (SellingPrice * Quantity * 1.04) AS 'Price with Proposed GST'
FROM Orders AS O
    INNER JOIN OrderDetails AS OD
...
-- 5. (3 Marks) We would like to know which city the majority of our northern Alberta customers are from. List the 
-- city and the total number of customers who placed an order from that city with postal codes starting with T5 to T9. 
-- List the cities alphabetically.
-- ANSWER_HERE
SELECT  City,
        COUNT(CustomerNumber) AS 'Customer Count'
FROM Customers
WHERE PostalCode LIKE 'T[5-9]%'
GROUP BY City
GO
...
-- ANSWER_HERE
CREATE OR ALTER VIEW ProductOrders
AS
...
-- ANSWER_HERE
SELECT  [Description],
        AVG(SellingPrice) AS 'Average Selling Price'
FROM ProductOrders
GROUP BY [Description]
