    ...
    ProductNumber       char(7)         
        CONSTRAINT FK_OrderDetails_ProductNumber_To_Products_ProductNumber
            REFERENCES Products (ProductNumber)
                                        NOT NULL,
    Quantity            int             
        CONSTRAINT CK_OrderDetials_Quainity
                                        NOT NULL,
...
CREATE NONCLUSTERED INDEX OrderDetails
ON OrderDetails(OrderNumber, ProductNumber)
...
ADD PrepaidCollect Char 
    CONSTRAINT CK_Delivers_PrepaidCollect    
        CHECK ( PrepaidCollect LIKE '[PC]')        
                    NULL
GO
...
SELECT SUM(Quantity) AS 'Total Quainty', [Description]
FROM OrderDetails AS O
    INNER JOIN Products AS P
    ON O.ProductNumber = P.ProductNumber
GROUP BY [Description]
HAVING SUM(Quantity) > 275
...
SELECT DriverFirstName + '' + DriverLastName AS 'Drivers Full Name', V.VehicleNumber, DeliveryNumber
FROM Vehicles AS V
    INNER JOIN Deliveries AS D
    ON V.VehicleNumber = D.VehicleNumber
WHERE DeliveryDate LIKE 'Sep %%, 2022'
...
SELECT SellingPrice * 1.05 AS 'Current GST', SellingPrice * 1.04 AS 'Proposed GTS'
FROM Orders AS O
...
SELECT City, COUNT(CustomerNumber) AS 'Total Customers'
FROM Customers
WHERE PostalCode LIKE 'T5%' OR PostalCode LIKE 'T6%' OR PostalCode LIKE 'T7%' OR PostalCode LIKE 'T8%' OR PostalCode LIKE 'T9%' 
GROUP BY City
...
SELECT [Description], AVG(SellingPrice) AS 'AVG Selling price'
FROM ProductOrders
GROUP BY [Description]
